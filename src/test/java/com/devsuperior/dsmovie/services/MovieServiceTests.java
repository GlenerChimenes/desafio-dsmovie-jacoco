...
public class MovieServiceTests {
	...
	void setup() throws Exception {
		...
		Mockito.when(repository.searchByTitle(movie.getTitle(), pageable)).thenReturn(page);
		...
		Mockito.when(repository.existsById(dependedMovieId)).thenReturn(true);
		
		Mockito.doNothing().when(repository).deleteById(existingId);
		Mockito.doThrow(DataIntegrityViolationException.class).when(repository).deleteById(dependedMovieId);
	...
	public void deleteShouldThrowDatabaseExceptionWhenDependentId() {
		Assertions.assertThrows(DatabaseException.class, () -> {
			service.delete(dependedMovieId);
		});
	}
}
